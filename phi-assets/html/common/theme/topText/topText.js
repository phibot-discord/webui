function themeTopText() {
	...
	for (let i = 0; i < textNum; ++i) {
		...
		pNode.style.fontSize = Math.floor(Math.random() * 20 + 10) + "px";
		pNode.style.filter = "blur(" + (Math.random() * 2 + 1) + "px)";
		pNode.innerText = textList[Math.floor(Math.random() * textList.length)];
