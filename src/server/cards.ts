...
> {
	...
	if (kind === "b30" || kind === "x30" || kind === "fc30") {
		const data = await host.lib.b19Card(
			host.rt,
			save,
			host.db,
			userId,
			catalog,
			{ nnum, mode: kind, locale },
		);
		return {
	...
	if (kind === "info") {
		const data = await host.lib.infoCard(
			host.rt,
			save,
			host.db,
			userId,
	...
	const snaps = await host.lib.loadHisb30Snaps(host.db, userId);
	const history = await host.lib.loadSaveHistory(host.rt, host.db, token);
	const rows = await host.lib.buildHisb30Rows(host.rt, history, snaps);
	if (!rows.length) {
	...
	return {
		...
		data: {
			...
			gameuser: host.lib.playerBlock(host.rt, save),
